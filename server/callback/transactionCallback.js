 ...
 */
export async function verifiedTransaction(error, result) {
  if (error) {
  ...
  try {
    const response = await axios.get(
      `${env.accountService}/api/v1/transaction/verify?transactionId=${result.transactionId}`, 
      {
        headers: {
    ...
    console.error('Failed to notify verify transaction:', error.message);
